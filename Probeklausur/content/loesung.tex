...
\begin{itemize}
\item [a)] $\mathbf{A} \cdot \mathbf{B} = \begin{pmatrix} 2 & 1 \\ 0 & 3 \end{pmatrix} \begin{pmatrix} 1 & -1 \\ 2 & 0 \end{pmatrix}$

Berechnung der Einträge:
\begin{align*}
(\mathbf{AB})_{11} &= 2 \cdot 1 + 1 \cdot 2 = 2 + 2 = 4 \\
(\mathbf{AB})_{12} &= 2 \cdot (-1) + 1 \cdot 0 = -2 + 0 = -2 \\
(\mathbf{AB})_{21} &= 0 \cdot 1 + 3 \cdot 2 = 0 + 6 = 6 \\
(\mathbf{AB})_{22} &= 0 \cdot (-1) + 3 \cdot 0 = 0 + 0 = 0
\end{align*}

Ergebnis: $\mathbf{A} \cdot \mathbf{B} = \begin{pmatrix} 4 & -2 \\ 6 & 0 \end{pmatrix}$ (2 Punkte)
\end{itemize}
...
In Worten: Die Ableitung des Zählers mal Nenner minus Zähler mal Ableitung des Nenners, geteilt durch Nenner zum Quadrat.

$u(x) = 1$, $v(x) = 1 + e^{-x}$
$u'(x) = 0$, $v'(x) = -e^{-x}$

$\sigma'(x) = \frac{0 \cdot (1 + e^{-x}) - 1 \cdot (-e^{-x})}{(1 + e^{-x})^2} = \frac{e^{-x}}{(1 + e^{-x})^2} = \frac{1}{1 + e^{-x}} \cdot \frac{e^{-x}}{1 + e^{-x}} = \sigma(x)(1 - \sigma(x))$
(3 Punkte)
\end{itemize}

\begin{itemize}
\item [b)] Berechnung von $\sigma(0)$:
\begin{align*}
\sigma(0) &= \frac{1}{1 + e^{0}} \\
&= \frac{1}{1 + 1} && \text{(da } e^0 = 1\text{)} \\
&= \frac{1}{2} \\
&= 0.5
\end{align*}
(2 Punkte)
...
Schritt 2: Berechne $a$:
\begin{align*}
a &= \sigma(z) = \sigma(0.5) \approx 0.62
\end{align*}
(0.5 Punkte)

Schritt 3: Berechne Sigmoid-Ableitung:
\begin{align*}
...
Schritt 4: Berechne Gradient mit Kettenregel:
\begin{align*}
\frac{\partial L}{\partial w} &= \frac{\partial L}{\partial a} \cdot \frac{\partial a}{\partial z} \cdot \frac{\partial z}{\partial w} \\
&= (a - y) \cdot \sigma'(z) \cdot x \\
&= (0.62 - 1) \cdot 0.236 \cdot 1 \\
&= (-0.38) \cdot 0.236 \\
&\approx -0.090
\end{align*}
(1 Punkt)
...
\begin{itemize}
\item [a)] Berechnung von $\mathbf{z}^H = \mathbf{W}^H \mathbf{x} + \mathbf{b}^H$:
\begin{align*}
\mathbf{z}^H &= \begin{pmatrix} 0.5 & 0.3 \\ 0.2 & 0.4 \end{pmatrix} \begin{pmatrix} 1 \\ 1 \end{pmatrix} + \begin{pmatrix} 0 \\ 0 \end{pmatrix} \\
z_1^H &= 0.5 \cdot 1 + 0.3 \cdot 1 + 0 = 0.5 + 0.3 = 0.8 \\
z_2^H &= 0.2 \cdot 1 + 0.4 \cdot 1 + 0 = 0.2 + 0.4 = 0.6 \\
\mathbf{z}^H &= \begin{pmatrix} 0.8 \\ 0.6 \end{pmatrix}
\end{align*}
(1 Punkt)
\end{itemize}
...
\begin{itemize}
\item [c)] Berechnung des Outputs $\hat{y} = (\mathbf{w}^O)^T \mathbf{a}^H + b^O$:
\begin{align*}
\hat{y} &= \begin{pmatrix} 1 & 1 \end{pmatrix} \begin{pmatrix} 0.8 \\ 0.6 \end{pmatrix} + 0 \\
&= 1 \cdot 0.8 + 1 \cdot 0.6 + 0 \\
&= 0.8 + 0.6 \\
&= 1.4
\end{align*}
(1.5 Punkte)
...
\begin{itemize}
\item [d)] Berechnung des Loss $L = \frac{1}{2}(y - \hat{y})^2$:
\begin{align*}
L &= \frac{1}{2}(2 - 1.4)^2 \\
&= \frac{1}{2}(0.6)^2 \\
&= \frac{1}{2} \cdot 0.36 \\
...
\begin{itemize}
\item [f)] Gradient für $w_1^O$ mit Kettenregel:
\begin{align*}
\frac{\partial L}{\partial w_1^O} &= \frac{\partial L}{\partial \hat{y}} \cdot \frac{\partial \hat{y}}{\partial w_1^O} \\
&= \frac{\partial L}{\partial \hat{y}} \cdot a_1^H && \text{(da } \frac{\partial \hat{y}}{\partial w_1^O} = a_1^H\text{)} \\
&= -0.6 \cdot 0.8 \\
&= -0.48
\end{align*}
(1.5 Punkte)
...
\begin{itemize}
\item [b)] Gewichts-Update mit $w_{\text{alt}} = 0.5$, $\eta = 0.1$, $\frac{\partial L}{\partial w} = 0.4$:
\begin{align*}
w_{\text{neu}} &= w_{\text{alt}} - \eta \frac{\partial L}{\partial w} \\
&= 0.5 - 0.1 \cdot 0.4 \\
&= 0.5 - 0.04 \\
...
Max-Pooling-Berechnung (wähle Maximum aus jedem $2 \times 2$ Block):
\begin{align*}
\text{Oben links:} \quad & \max\{1, 2, 5, 6\} = 6 \\
\text{Oben rechts:} \quad & \max\{3, 4, 7, 8\} = 8 \\
\text{Unten links:} \quad & \max\{9, 10, 13, 14\} = 14 \\
\text{Unten rechts:} \quad & \max\{11, 12, 15, 16\} = 16
\end{align*}

Ausgabe: $\begin{pmatrix} 6 & 8 \\ 14 & 16 \end{pmatrix}$
...
\begin{itemize}
\item [d)] Berechnung des Gradienten mit $\hat{y} = 0.75$, $y = 2$, $\mathbf{a} = \begin{pmatrix} 1 \\ 0.5 \end{pmatrix}$:
\begin{align*}
\frac{\partial L}{\partial \mathbf{w}} &= (\hat{y} - y) \cdot \mathbf{a} \\
&= (0.75 - 2) \cdot \begin{pmatrix} 1 \\ 0.5 \end{pmatrix} \\
&= (-1.25) \cdot \begin{pmatrix} 1 \\ 0.5 \end{pmatrix} \\
&= \begin{pmatrix} -1.25 \cdot 1 \\ -1.25 \cdot 0.5 \end{pmatrix} \\
&= \begin{pmatrix} -1.25 \\ -0.625 \end{pmatrix}
\end{align*}
(2 Punkte)
\end{itemize}
